...
RSpec.describe 'BankAccount class' do
  ...
  describe '#withdraw' do
    context 'given a negative amount and a date' do
      let(:date) { Time.new(2023, 7, 19) }
    ...
    context 'given a negative amount with no date' do
      it 'defaults to the current date' do
        today = Time.new(2023, 7, 19)
    ...
    context 'given a positive amount' do
      it 'raises an error "amount must be below zero"' do
        expect { account.withdraw(1) }.to raise_error 'amount must be below zero'
      end
    end

    context 'given 0' do
      it 'raises an error "amount must be below zero"' do
        expect { account.withdraw(0) }.to raise_error 'amount must be below zero'
      end
    end   
  end
end
